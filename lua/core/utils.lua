...
M.close_buffer = function(force)
   ...
   local function switch_buffer(windows, buf)
      ...
      cmd(string.format("%d wincmd w", cur_win)) -- return to original window
   end
   ...
   if #vim.fn.getbufinfo { buflisted = 1 } < 2 then
      ...
      if chad_term then
         ...
         cmd(string.format("setlocal nobl", buf))
         cmd "enew"
         return
      end
   ...
   if force or vim.fn.getbufvar(buf, "&buftype") == "terminal" then
      ...
      if chad_term then
         if type == "wind" then
            ...
            cmd(string.format("%d bufdo setlocal nobl", buf))
            ...
            cmd(string.format("%d wincmd c", cur_win))
            return
         end
      else
...
-- hide statusline
-- tables fetched from plugins/plugin_settings.lua
M.hide_statusline = function()
   local hidden = require("plugins.plugin_settings").options.statusline.hidden
   local shown = require("plugins.plugin_settings").options.statusline.shown
   local api = vim.api
